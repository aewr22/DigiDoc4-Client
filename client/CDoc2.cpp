...
namespace cdoc20 {
	...
	struct stream final: public QIODevice
	{
		...
		qint64 readData(char *data, qint64 maxlen) final
		{
			...
			{
				if(auto size = io->read(in.data(), CHUNK); size > 0)
				{
		...
		qint64 writeData(const char *data, qint64 len) final
		{
			...
			while(true) {
				...
				if(res == Z_STREAM_ERROR)
					return -1;
				if(auto size = CHUNK - s.avail_out; size > 0)
				{
	...
	: QFile(path)
{
	...
	for(auto recipient: *recipients){
		...
		{
			if(auto key = recipient->capsule_as_ECCPublicKeyCapsule())
			{
		...
		{
			if(auto key = recipient->capsule_as_RSAPublicKeyCapsule())
			{
		...
		{
			auto server = recipient->capsule_as_KeyServerCapsule();
			if(!server)
			...
			{
				if(auto eccDetails = server->recipient_key_details_as_ServerEccDetails())
				{
			...
			{
				if(auto rsaDetails = server->recipient_key_details_as_ServerRsaDetails())
...
bool CDoc2::save(const QString &path)
{
	...
	auto sendToServer = [this](CKey &key, const QByteArray &recipient_id, const QByteArray &key_material, QLatin1String type) {
		...
		QScopedPointer<QNetworkAccessManager,QScopedPointerDeleteLater> nam(CheckConnection::setupNAM(req));
		QEventLoop e;
...
QByteArray CDoc2::transportKey(const CKey &_key)
{
	...
	if(!key.transaction_id.isEmpty())
	{
		...
		QScopedPointer<QNetworkAccessManager,QScopedPointerDeleteLater> nam(
			CheckConnection::setupNAM(req, qApp->signer()->tokenauth().cert(), authKey));
		QEventLoop e;
