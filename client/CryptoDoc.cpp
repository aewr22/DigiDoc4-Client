...
void CDocumentModel::open(int row)
{
	...
	if(FileDialog::isSignedPDF(f.absoluteFilePath()))
		Application::showClient({ f.absoluteFilePath() }, false, false, true);
	else
...
bool CryptoDoc::decrypt()
{
	...
	if(d->cdoc->version() == 2 && !Settings::CDOC2_NOTIFICATION.isSet())
	{
		auto *dlg = WarningDialog::show(tr("You must enter your PIN code twice in order to decrypt the CDOC2 container. "
	...
	if(d->key.isEmpty())
	{
		WarningDialog::show(tr("Failed to retrieve transport key"), d->cdoc->lastError);
		return false;
	}
