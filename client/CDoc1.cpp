	...
	: QFile(path)
{
	...
	if(files.empty() && properties.contains(QStringLiteral("Filename")))
	{
		files.push_back({
			...
			{},
			mime == MIME_ZLIB ? properties[QStringLiteral("OriginalMimeType")] : mime,
			properties[QStringLiteral("OriginalSize")].toUInt(),
		});
	}
}
...
bool CDoc1::decryptPayload(const QByteArray &key)
{
	...
	if(data.isEmpty())
	{
		lastError = CryptoDoc::tr("Failed to decrypt document");
		return false;
	}
	...
	if(mime == MIME_ZLIB)
	{
		...
		unsigned origsize = std::max<unsigned>(properties[QStringLiteral("OriginalSize")].toUInt(), 1);
							...
							writeElement(w, DENC, QStringLiteral("AgreementMethod"), {
								...
								writeElement(w, XENC11, QStringLiteral("KeyDerivationMethod"), {
									...
									writeElement(w, XENC11, QStringLiteral("ConcatKDFParams"), {
										{QStringLiteral("AlgorithmID"), QStringLiteral("00") + props.value("DocumentFormat").toUtf8().toHex()},
...
void CDoc1::writeDDoc(QIODevice *ddoc)
{
	...
	for(const File &file: qAsConst(files))
	{
		...
		file.data->seek(0);
		for(int size = 0; (size = file.data->read(buf.data(), buf.size())) > 0;)
